...
function makePendingApptCard(appt) {
    ...
    const apptElem = `
      <div class="card" id="appt-${appt['appt_id']}">
        ...
        <p>Approved: ${!appt.approved ? '<button onclick="markApproved(' + appt.appt_id + ')">Mark Approved</button>' : 'Yes'}</p>
        <p>${appt.notes}</p>
      </div>
    `;
...
function makeApptCard(appt, nextAppt) {
    ...
    const markCompletedButton = !appt.completed ? `<button onclick="markCompleted(${appt.appt_id})">Mark Completed</button>` : 'yes';
    const markIncompleteButton = appt.completed ? `<button onclick="markIncomplete(${appt.appt_id})">Mark Incomplete</button>` : '';
    const apptElem = `
        <div class="card" id="appt-${appt['appt_id']}">
            <h2>${dateDisplayText}</h2>
            <h3>${appt['service_type']}</h3>
            ...
            <p>Approved: yes | Completed: ${markCompletedButton} ${markIncompleteButton}</p>
            <p>${appt.notes}</p>
        </div>
    `;
    return apptElem;
...
function markIncomplete(id) {
    ...
    axios.put('http://localhost:8765/incomplete', { apptId: id })
      .then(() => getUpcomingAppointments())
      .catch(err => console.log(err));
}
